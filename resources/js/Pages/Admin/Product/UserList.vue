    ...
    <div>
      ...
      >
        ...
        <div class="field">
          ...
          <InputText id="email" v-model="user.email" required="true" rows="3" cols="20" />
        </div>
...
const updateProduct = async () => {
  ...
  formData.append("isAdmin", user.value.isAdmin);
  formData.append("_method", "PUT");
  // Append product images to the FormData

  try {
    await router.post("users/update/" + user.value.id, formData, {
      onSuccess: (page) => {
        Swal.fire({
          toast: true,
          icon: "success",
...
const deleteProduct = () => {
  try {
    router.delete("users/destroy/" + user.value.id, {
      onSuccess: (page) => {
        Swal.fire({
          toast: true,
          icon: "success",
